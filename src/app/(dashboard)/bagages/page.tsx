...
export default function BagagesPage() {
  ...
  const [form, setForm] = useState({ date_bagage: new Date().toISOString().split('T')[0], gare_id: '', montant: '' })

  useEffect(() => { charger() }, [])

  async function charger(debut?: string, fin?: string) {
    let q = supabase.from('bagages').select('*, gare:gares(nom)').order('date_bagage', { ascending: false })
    if (debut) q = q.gte('date_bagage', debut)
    if (fin) q = q.lte('date_bagage', fin)
  ...
  async function sauvegarder() {
    ...
    setSaving(true)
    const payload = { date_bagage: form.date_bagage, gare_id: form.gare_id || null, montant: parseInt(form.montant) || 0 }
    if (editId) await supabase.from('bagages').update(payload).eq('id', editId)
    else await supabase.from('bagages').insert(payload)
    setForm({ date_bagage: new Date().toISOString().split('T')[0], gare_id: '', montant: '' })
    setShowForm(false); setEditId(null); setSaving(false); charger(dateDebut, dateFin)
  }

  function editer(c: Bagage) {
    setForm({ date_bagage: c.date_bagage, gare_id: c.gare_id || '', montant: String(c.montant) })
    setEditId(c.id); setShowForm(true)
  }

  async function supprimer(id: string) {
    if (!confirm('Supprimer ce bagage ?')) return
    await supabase.from('bagages').delete().eq('id', id); charger(dateDebut, dateFin)
  }

  const fmt = (n: number) => n.toLocaleString('fr-FR') + ' F'
  const total = bagages.reduce((s, c) => s + c.montant, 0)
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1a3a2a] flex items-center gap-2">
            <Package className="w-6 h-6 text-amber-500" /> Bagages
          </h1>
          <p className="text-zinc-400 text-sm mt-1">Recettes journalières — service bagages</p>
        </div>
        <button onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ date_bagage: new Date().toISOString().split('T')[0], gare_id: '', montant: '' }) }}
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-4 mb-6 flex gap-3 items-end flex-wrap">
        ...
        <div className="ml-auto bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 text-right">
          <p className="text-xs text-amber-400 font-medium">Total période</p>
          <p className="text-lg font-bold text-amber-600">{fmt(total)}</p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-5 mb-6 shadow-sm">
          <h2 className="font-semibold text-zinc-700 text-sm mb-4">{editId ? 'Modifier le bagage' : 'Nouveau bagage'}</h2>
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div>
              <label className="text-xs text-zinc-500 font-medium mb-1 block">Date</label>
              <input type="date" value={form.date_bagage} onChange={e => setForm({ ...form, date_bagage: e.target.value })}
      ...
      {/* Tableau */}
      {loading ? <p className="text-zinc-400 text-sm">Chargement...</p> : bagages.length === 0 ? (
        <div className="bg-white border border-zinc-200 rounded-2xl p-12 text-center">
          <Package className="w-12 h-12 text-zinc-300 mx-auto mb-3" />
          <p className="text-zinc-400 text-sm">Aucun bagage enregistré</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {bagages.map(c => (
                <tr key={c.id} className="border-t border-zinc-100 hover:bg-zinc-50">
                  <td className="px-4 py-3 text-zinc-500">{new Date(c.date_bagage).toLocaleDateString('fr-FR')}</td>
                  <td className="px-4 py-3">{c.gare?.nom || <span className="text-zinc-300">—</span>}</td>
                  <td className="px-4 py-3 font-bold text-amber-600">{fmt(c.montant)}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => editer(c)} className="text-zinc-300 hover:text-amber-500 transition-colors"><Pencil className="w-4 h-4" /></button>
            ...
            <tfoot className="bg-zinc-50 border-t-2 border-zinc-200">
              <tr>
                ...
                <td className="px-4 py-3 font-bold text-amber-600">{fmt(total)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
