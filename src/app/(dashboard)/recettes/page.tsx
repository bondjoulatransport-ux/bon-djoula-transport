...
export default function RecettesPage() {
  ...
  const [form, setForm] = useState({ date_recette: '', vehicule_id: '', gare_id: '', recette_gare: '', recette_route: '' })
  ...
  async function charger() {
    const { data: r } = await supabase.from('recettes').select('*, vehicule:vehicules(plaque), gare:gares(nom)').order('date_recette', { ascending: false })
  ...
  async function ajouter() {
    ...
    setForm({ date_recette: '', vehicule_id: vehicules[0]?.id || '', gare_id: gares[0]?.id || '', recette_gare: '', recette_route: '' })
    setShow(false)
    charger()
  }
      ...
      {loading ? <p className="text-zinc-400 text-sm">Chargement...</p> : (
        <div className="bg-white border border-zinc-200 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {recettes.length === 0 && <tr><td colSpan={6} className="px-4 py-6 text-zinc-400 text-center">Aucune recette — cliquez sur Saisir recette</td></tr>}
              {recettes.map(r => (
