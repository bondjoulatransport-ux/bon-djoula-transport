  ...
  return (
    <aside className="w-56 min-h-screen bg-[#1a3a2a] flex flex-col">
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
        <div className="w-9 h-9 bg-[#f7b731] rounded-full flex items-center justify-center flex-shrink-0">
          <Bus className="w-5 h-5 text-[#1a3a2a]" />
        </div>
        <div>
          <p className="text-white text-sm font-semibold leading-tight">Bon Djoula</p>
        ...
        {nav.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={cn(
              ...
              active ? 'bg-[#f7b731] text-[#1a3a2a] font-medium' : 'text-[#8fbf9f] hover:bg-white/10 hover:text-white'
            )}>
